...
public class Linpack implements SapphireObject<OahuPolicy> {

  public AndyMath getAndyMath() {
	  return (AndyMath) new_(AndyMath.class);
  }
  
  public Sudoku getSudoku() {
	return (Sudoku) new_(Sudoku.class);
  }
  
  public ImageRec getImageRec() {
	  return (ImageRec) new_(ImageRec.class);
  }
  
  
  public WorldHarness getPhysics() {
	  return (WorldHarness) new_(WorldHarness.class);
  }
  
  public aichess4k getChessAI() {
	  return (aichess4k) new_(aichess4k.class);
  }
  ...
  public static class Result implements Serializable{
    ...
    public Result(double mflops, double timems, double residn, double precision) {
      this.mflops = mflops; this.timems = timems; this.residn = residn; this.precision = precision;
    }
    public Result() { this(0.0,0.0,0.0,0.0);}
    
    public String toString() {
    	return ("Mflops/s: " + mflops +
    		    "\nTime: " + timems + " ms");
    }
  }
  ...
  public Result run(int n) {
	  Result out = new Result();
	  for(int i = 0; i < n; i++) {
		  Result temp = this.run_benchmark();
		  out.mflops += temp.mflops;
		  out.precision = temp.precision;
		  out.residn += temp.residn;
		  out.timems += temp.timems;
	  }
	  out.mflops /= n;
	  out.residn /= n;
	  return out;
...
*/
    ...
    return new Result(mflops_result,time_result, residn_result, eps_result);
  }
