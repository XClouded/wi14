 ...
 */
public class Sudoku implements SapphireObject<OahuPolicy>{
    ...
    public int[][] solve(int[][] matrix) {
        ...
        if (solve(0,0,matrix)) {    // solves in place
        	...
        	System.out.println("Execution time (couldn't solve):	" + (System.nanoTime() - start));
        	return null;
        }
    ...
    boolean solve(int i, int j, int[][] cells) {
    	//System.out.println("examining " + i +"," + j);
        if (i == 9) {
            i = 0;
            if (++j == 9)
                return true;
