...
public final class AppStub extends Stub {
	...
	public String getStubAdditionalMethods() {
		...
		/* The $__getReference function */
		buffer.append(indenter.indent() + "public sapphire.policy.SapphirePolicy.SapphireClientPolicy $__getReference() {" + EOLN);
	...
	public String getMethodContent(MethodStub m) {
		...
		for (int i = 0; i < m.numParams; i++) {
			listParams.append(indenter.tIncrease(2) + "$__params[" + i + "] = " + m.paramNames[i] + ";" + EOLN);
		...
		if (m.numParams > 0)
			buffer.append(indenter.tIncrease(2) + "Object[] $__params = new Object[" + m.numParams + "];" + EOLN); //$NON-NLS-1$
		else
			buffer.append(indenter.tIncrease(2) + "Object[] $__params = null;" + EOLN); //$NON-NLS-1$
