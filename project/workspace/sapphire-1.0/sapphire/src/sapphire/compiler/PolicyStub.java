...
import sapphire.compiler.util.RmicUtil;
...
public class PolicyStub extends Stub {
	...
	public String getStubFields() {
		StringBuilder buffer = new StringBuilder();
		buffer.append(indenter.indent() + "private static final long serialVersionUID = 2L;" + EOLN);
	...
	public String getStubAdditionalMethods() {
		...
		buffer.append(indenter.indent() + "public Object $__makeKernelRPC(java.lang.String method, Object[] params) throws java.rmi.RemoteException, java.lang.Exception {" + EOLN);
		...
		buffer.append(indenter.tIncrease(2) + "return sapphire.kernel.common.GlobalKernelReferences.nodeServer.getKernelClient().makeKernelRPC(this, $__oid, method, params);" + EOLN);
	...
	public String getMethodContent(MethodStub m) {
		...
		if (m.numParams > 0) {
			...
			buffer.append(indenter.indent() + "Object[] $__params = new Object[" + m.numParams + "];" + EOLN); //$NON-NLS-1$
			for (int i = 0; i < m.numParams; i++) {
				buffer.append(indenter.indent() + "$__params[" + i + "] = " + m.paramNames[i] + ";" + EOLN);
			}
		}
		else
			buffer.append(indenter.indent() + "Object[] $__params = null;" + EOLN); //$NON-NLS-1$
